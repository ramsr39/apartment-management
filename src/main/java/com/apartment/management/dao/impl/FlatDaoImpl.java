...
public class FlatDaoImpl extends NamedParameterJdbcDaoSupport implements FlatDao {
  ...
  private static final String FLAT_ID_COUNT_QUERY =
    "SELECT count(FLAT_ID) from flatunit WHERE BUILDING_ID=:BUILDING_ID";
  ...
  public List<FlatDTO> findFlatDetailsByBuildingId(final String buildingId) {
    ...
    return getNamedParameterJdbcTemplate().query(FIND_FLAT_DETAILS, mapSqlParameterSource, new RowMapper<FlatDTO>() {
      @Override
  ...
  private UserDTO getFlatOwnerDetails(final String ownerId) {
    ...
    return getNamedParameterJdbcTemplate().queryForObject(GET_FLAT_OWNER_QUERY, mapSqlParameterSource,
  ...
  public List<CoOccupantDTO> findCoOccupants(final String flatId, final String userId) {
    try {
      ...
      return getNamedParameterJdbcTemplate().query(FIND_CO_OCCUPANTS_FOR_FLAT_USER_QUERY, mapSqlParameterSource,
          new RowMapper<CoOccupantDTO>() {
            @Override
            public CoOccupantDTO mapRow(final ResultSet rs, final int rowNum) throws SQLException {
              ...
              coOccupantDto.setId(rs.getString("CO_OCCUPANT_ID"));
  ...
  private UserDTO getFaltTenantDetails(final String ownerId) {
    ...
    try {
      ...
      return getNamedParameterJdbcTemplate().queryForObject(GET_FLAT_TENANT_QUERY, mapSqlParameterSource,
          new RowMapper<UserDTO>() {
            @Override
            public UserDTO mapRow(final ResultSet rs, final int rowNum) throws SQLException {
      ...
      LOG.info("found empty coocupants for the user::" + ownerId + "::" + er);
      return new UserDTO();
    }
  }
}
